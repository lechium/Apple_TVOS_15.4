//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class ACAccount, AMSProcessInfo, NSNumber, NSString;

@interface IAPInfoUpdateRequestContext : NSObject
{
    ACAccount *_account;	// 8 = 0x8
    AMSProcessInfo *_clientInfo;	// 16 = 0x10
    NSString *_logUUID;	// 24 = 0x18
    NSNumber *_revision;	// 32 = 0x20
}

+ (id)newContextWithRevision:(id)arg1 forAccount:(id)arg2 token:(id)arg3;	// IMP=0x0040000000254177
- (void).cxx_destruct;	// IMP=0x002000000025438d
// Error: Property attributes should begin with the type ('T') attribute, property name: revision
// Property attributes: (null)

@property(readonly) NSString *logUUID; // @synthesize logUUID=_logUUID;
@property(readonly) AMSProcessInfo *clientInfo; // @synthesize clientInfo=_clientInfo;
@property(readonly) ACAccount *account; // @synthesize account=_account;
- (id)_initWithRevision:(id)arg1 forAccount:(id)arg2 client:(id)arg3 logUUID:(id)arg4;	// IMP=0x001000000025425b

@end

