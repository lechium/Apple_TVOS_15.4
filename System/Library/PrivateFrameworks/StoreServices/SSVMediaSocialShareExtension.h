//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSString, SSXPCConnection;

@interface SSVMediaSocialShareExtension : NSObject
{
    SSXPCConnection *_connection;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x00000000000ed52c
- (void)getVisibilityWithCompletionBlock:(CDUnknownBlockType)arg1;	// IMP=0x00000000000ed306
@property(readonly, copy, nonatomic) NSString *extensionIdentifier;
- (id)init;	// IMP=0x00000000000ed289

@end

