//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@interface RBProcessCPUMaximumLimits : NSObject
{
    unsigned long long _percentage;	// 8 = 0x8
    unsigned long long _duration;	// 16 = 0x10
    unsigned long long _violationPolicy;	// 24 = 0x18
}

@property(readonly, nonatomic) unsigned long long violationPolicy; // @synthesize violationPolicy=_violationPolicy;
@property(readonly, nonatomic) unsigned long long duration; // @synthesize duration=_duration;
@property(readonly, nonatomic) unsigned long long percentage; // @synthesize percentage=_percentage;
- (id)description;	// IMP=0x000000000001c7dc
- (unsigned long long)hash;	// IMP=0x000000000001c744
- (_Bool)isEqual:(id)arg1;	// IMP=0x000000000001c6cf
- (id)unionLimit:(id)arg1;	// IMP=0x000000000001c5d2
- (id)initWithPercentage:(unsigned long long)arg1 duration:(unsigned long long)arg2 violationPolicy:(unsigned long long)arg3;	// IMP=0x000000000001c57f

@end

