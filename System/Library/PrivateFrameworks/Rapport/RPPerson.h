//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <Rapport/NSSecureCoding-Protocol.h>

@class CURangingMeasurement, NSArray, NSMutableDictionary, NSString;

@interface RPPerson : NSObject <NSSecureCoding>
{
    int _activityLevel;	// 8 = 0x8
    unsigned int _flags;	// 12 = 0xc
    int _proximity;	// 16 = 0x10
    NSString *_contactID;	// 24 = 0x18
    NSArray *_devices;	// 32 = 0x20
    NSString *_identifier;	// 40 = 0x28
    NSString *_name;	// 48 = 0x30
    CURangingMeasurement *_relativeLocation;	// 56 = 0x38
    NSMutableDictionary *_deviceDict;	// 64 = 0x40
}

+ (_Bool)supportsSecureCoding;	// IMP=0x0000000000048406
- (void).cxx_destruct;	// IMP=0x00000000000492f3
@property(retain, nonatomic) NSMutableDictionary *deviceDict; // @synthesize deviceDict=_deviceDict;
@property(readonly) CURangingMeasurement *relativeLocation; // @synthesize relativeLocation=_relativeLocation;
@property(readonly, nonatomic) int proximity; // @synthesize proximity=_proximity;
@property(readonly, copy, nonatomic) NSString *name; // @synthesize name=_name;
@property(copy, nonatomic) NSString *identifier; // @synthesize identifier=_identifier;
@property(readonly, nonatomic) unsigned int flags; // @synthesize flags=_flags;
@property(readonly, copy, nonatomic) NSArray *devices; // @synthesize devices=_devices;
@property(readonly, copy, nonatomic) NSString *contactID; // @synthesize contactID=_contactID;
@property(readonly, nonatomic) int activityLevel; // @synthesize activityLevel=_activityLevel;
- (unsigned int)_updateDeviceDerivedInfo;	// IMP=0x0000000000048ea5
- (unsigned int)updateWithRPDevice:(id)arg1;	// IMP=0x0000000000048d98
- (unsigned int)removeRPDevice:(id)arg1;	// IMP=0x0000000000048ce0
- (id)descriptionWithLevel:(int)arg1;	// IMP=0x0000000000048786
- (id)description;	// IMP=0x000000000004876f
- (void)encodeWithCoder:(id)arg1;	// IMP=0x000000000004864d
- (id)initWithCoder:(id)arg1;	// IMP=0x000000000004840e

@end

