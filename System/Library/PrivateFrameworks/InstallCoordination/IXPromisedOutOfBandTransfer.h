//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <InstallCoordination/NSSecureCoding-Protocol.h>

@class IXPromisedOutOfBandTransferSeed;

@interface IXPromisedOutOfBandTransfer <NSSecureCoding>
{
}

+ (_Bool)supportsSecureCoding;	// IMP=0x000000000003ad3c
- (Class)seedClass;	// IMP=0x000000000003ad73
- (id)initWithSeed:(id)arg1;	// IMP=0x000000000003ad44
- (void)encodeWithCoder:(id)arg1;	// IMP=0x000000000003ad0d
- (id)initWithCoder:(id)arg1;	// IMP=0x000000000003acde
- (id)initWithName:(id)arg1 client:(unsigned long long)arg2 diskSpaceNeeded:(unsigned long long)arg3;	// IMP=0x000000000003a916

// Remaining properties
@property(nonatomic, getter=isComplete) _Bool complete; // @dynamic complete;
@property(nonatomic) double percentComplete; // @dynamic percentComplete;
@property(retain, nonatomic) IXPromisedOutOfBandTransferSeed *seed; // @dynamic seed;

@end

