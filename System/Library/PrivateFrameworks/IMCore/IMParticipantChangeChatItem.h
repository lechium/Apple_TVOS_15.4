//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class IMHandle, NSUUID;

@interface IMParticipantChangeChatItem
{
    IMHandle *_sender;	// 40 = 0x28
    IMHandle *_otherHandle;	// 48 = 0x30
    NSUUID *_activeTelephonyConversationUUID;	// 56 = 0x38
}

- (void).cxx_destruct;	// IMP=0x0000000000078788
@property(retain, nonatomic) NSUUID *activeTelephonyConversationUUID; // @synthesize activeTelephonyConversationUUID=_activeTelephonyConversationUUID;
@property(readonly, nonatomic) IMHandle *otherHandle; // @synthesize otherHandle=_otherHandle;
@property(readonly, nonatomic) IMHandle *sender; // @synthesize sender=_sender;
- (id)_initWithItem:(id)arg1 sender:(id)arg2 otherHandle:(id)arg3 context:(id)arg4;	// IMP=0x00000000000785bd
- (id)_initWithItem:(id)arg1 sender:(id)arg2 otherHandle:(id)arg3;	// IMP=0x000000000007851f
@property(readonly, nonatomic) long long changeType;
- (id)copyWithZone:(struct _NSZone *)arg1;	// IMP=0x0000000000078460

@end

