//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class NSArray, SAUIAppPunchOut, SAUIImageResource;

@interface SAWeatherAirQualitySnippet
{
}

+ (id)airQualitySnippetWithDictionary:(id)arg1 context:(id)arg2;	// IMP=0x0000000000015e42
+ (id)airQualitySnippet;	// IMP=0x0000000000015e30
@property(retain, nonatomic) SAUIImageResource *attributionImage;
@property(retain, nonatomic) SAUIAppPunchOut *appPunchOut;
@property(copy, nonatomic) NSArray *aceAirQualities;
- (id)encodedClassName;	// IMP=0x0000000000015e23
- (id)groupIdentifier;	// IMP=0x0000000000015e0f

@end

