//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSArray;

@interface FAInviteCompletionInfo : NSObject
{
    unsigned long long _transportType;	// 8 = 0x8
    NSArray *_recipients;	// 16 = 0x10
    unsigned long long _status;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x000000000000e790
@property(nonatomic) unsigned long long status; // @synthesize status=_status;
@property(retain, nonatomic) NSArray *recipients; // @synthesize recipients=_recipients;
@property(nonatomic) unsigned long long transportType; // @synthesize transportType=_transportType;
- (id)_completionStatusStringValue;	// IMP=0x000000000000e70c
- (id)_transportStringValue;	// IMP=0x000000000000e6a8
- (id)serverReadableDictionary;	// IMP=0x000000000000e5b8

@end

