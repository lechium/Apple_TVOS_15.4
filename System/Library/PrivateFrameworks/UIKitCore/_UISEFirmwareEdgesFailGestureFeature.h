//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <UIKitCore/_UISETouchedEdgesProvider-Protocol.h>

@protocol _UISEGestureFeatureSettings;

__attribute__((visibility("hidden")))
@interface _UISEFirmwareEdgesFailGestureFeature <_UISETouchedEdgesProvider>
{
    id <_UISEGestureFeatureSettings> _settings;	// 8 = 0x8
    _Bool _hasDoneTest;	// 16 = 0x10
    unsigned long long _touchedEdges;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x00000000006a05ed
@property(readonly, nonatomic) unsigned long long touchedEdges; // @synthesize touchedEdges=_touchedEdges;
- (void)_incorporateSample:(const struct _UISEGestureFeatureSample *)arg1;	// IMP=0x00000000006a053a
- (id)debugDictionary;	// IMP=0x00000000006a048b
- (id)initWithSettings:(id)arg1;	// IMP=0x00000000006a0403

@end

