//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <AssistantUI/NSCopying-Protocol.h>

@class FBSDisplayConfiguration, FBSSceneParameters;

@interface AFUISceneConfiguration : NSObject <NSCopying>
{
    FBSDisplayConfiguration *_displayConfiguration;	// 8 = 0x8
    _Bool _foreground;	// 16 = 0x10
    _Bool _cancelSceneLoadingAfterTimeout;	// 17 = 0x11
    long long _launchIntent;	// 24 = 0x18
    long long _preferredDeferralMode;	// 32 = 0x20
    double _sceneLoadingTimeOutDuration;	// 40 = 0x28
    FBSSceneParameters *_sceneParameters;	// 48 = 0x30
    long long _userInterfaceStyle;	// 56 = 0x38
    unsigned long long _deactivationReasonMask;	// 64 = 0x40
    struct CGRect _initialBounds;	// 72 = 0x48
    struct UIEdgeInsets _initialSafeAreaInsets;	// 104 = 0x68
    struct CGRect _bounds;	// 136 = 0x88
}

+ (id)defaultSiriSceneConfigurationWithInitialBounds:(struct CGRect)arg1 onDisplay:(id)arg2;	// IMP=0x000000000001c66d
+ (id)stringForInvalidationReason:(unsigned long long)arg1;	// IMP=0x000000000001c4be
- (void).cxx_destruct;	// IMP=0x000000000001ca2e
@property(nonatomic) struct CGRect bounds; // @synthesize bounds=_bounds;
@property(nonatomic) unsigned long long deactivationReasonMask; // @synthesize deactivationReasonMask=_deactivationReasonMask;
@property(nonatomic) long long userInterfaceStyle; // @synthesize userInterfaceStyle=_userInterfaceStyle;
@property(copy, nonatomic) FBSSceneParameters *sceneParameters; // @synthesize sceneParameters=_sceneParameters;
@property(nonatomic) struct UIEdgeInsets initialSafeAreaInsets; // @synthesize initialSafeAreaInsets=_initialSafeAreaInsets;
@property(readonly, nonatomic) struct CGRect initialBounds; // @synthesize initialBounds=_initialBounds;
@property(nonatomic) double sceneLoadingTimeOutDuration; // @synthesize sceneLoadingTimeOutDuration=_sceneLoadingTimeOutDuration;
@property(nonatomic) _Bool cancelSceneLoadingAfterTimeout; // @synthesize cancelSceneLoadingAfterTimeout=_cancelSceneLoadingAfterTimeout;
@property(nonatomic) long long preferredDeferralMode; // @synthesize preferredDeferralMode=_preferredDeferralMode;
@property(nonatomic) long long launchIntent; // @synthesize launchIntent=_launchIntent;
@property(nonatomic) _Bool foreground; // @synthesize foreground=_foreground;
- (id)description;	// IMP=0x000000000001c7f2
- (id)copyWithZone:(struct _NSZone *)arg1;	// IMP=0x000000000001c6d7
- (void)_setupPlatformSpecificDefaultSettings;	// IMP=0x000000000001c55c
- (void)configureMutableSceneSettings:(id)arg1;	// IMP=0x000000000001c2d4
- (id)_initWithInitialBounds:(struct CGRect)arg1 displayConfiguration:(id)arg2;	// IMP=0x000000000001befe
- (id)init;	// IMP=0x000000000001be6c

@end

