//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class NSArray, NSData, NSNumber, NSString;

@interface MCMDMPayload
{
    _Bool _useDevelopmentAPNS;	// 80 = 0x50
    _Bool _pinningRevocationCheckRequired;	// 81 = 0x51
    _Bool _signMessage;	// 82 = 0x52
    _Bool _checkOutWhenRemoved;	// 83 = 0x53
    int _accessRights;	// 84 = 0x54
    NSString *_identityUUID;	// 88 = 0x58
    NSData *_identityPersistentID;	// 96 = 0x60
    NSString *_topic;	// 104 = 0x68
    NSString *_serverURLString;	// 112 = 0x70
    NSArray *_serverPinningUUIDs;	// 120 = 0x78
    NSArray *_serverPinningPersistentRefs;	// 128 = 0x80
    NSString *_checkInURLString;	// 136 = 0x88
    NSArray *_checkInPinningUUIDs;	// 144 = 0x90
    NSArray *_checkInPinningPersistentRefs;	// 152 = 0x98
    NSArray *_serverCapabilities;	// 160 = 0xa0
    NSString *_managedAppleID;	// 168 = 0xa8
    NSString *_assignedManagedAppleID;	// 176 = 0xb0
    NSString *_enrollmentMode;	// 184 = 0xb8
    NSString *_personaID;	// 192 = 0xc0
    NSString *_enrollmentID;	// 200 = 0xc8
    NSString *_iCloudEnrollmentID;	// 208 = 0xd0
    NSString *_easEnrollmentID;	// 216 = 0xd8
    NSNumber *_pollingIntervalMinutes;	// 224 = 0xe0
    NSNumber *_requiredAppIDForMDM;	// 232 = 0xe8
    NSNumber *_useDevelopmentAPNSNum;	// 240 = 0xf0
    NSNumber *_signMessageNum;	// 248 = 0xf8
    NSNumber *_checkOutWhenRemovedNum;	// 256 = 0x100
}

+ (id)localizedPluralForm;	// IMP=0x000000000003fc8f
+ (id)localizedSingularForm;	// IMP=0x000000000003fc7c
+ (id)typeStrings;	// IMP=0x000000000003fc5c
- (void).cxx_destruct;	// IMP=0x000000000004375f
@property(readonly, nonatomic) NSNumber *checkOutWhenRemovedNum; // @synthesize checkOutWhenRemovedNum=_checkOutWhenRemovedNum;
@property(readonly, nonatomic) NSNumber *signMessageNum; // @synthesize signMessageNum=_signMessageNum;
@property(readonly, nonatomic) NSNumber *useDevelopmentAPNSNum; // @synthesize useDevelopmentAPNSNum=_useDevelopmentAPNSNum;
@property(readonly, nonatomic) NSNumber *requiredAppIDForMDM; // @synthesize requiredAppIDForMDM=_requiredAppIDForMDM;
@property(readonly, retain, nonatomic) NSNumber *pollingIntervalMinutes; // @synthesize pollingIntervalMinutes=_pollingIntervalMinutes;
@property(retain, nonatomic) NSString *easEnrollmentID; // @synthesize easEnrollmentID=_easEnrollmentID;
@property(retain, nonatomic) NSString *iCloudEnrollmentID; // @synthesize iCloudEnrollmentID=_iCloudEnrollmentID;
@property(retain, nonatomic) NSString *enrollmentID; // @synthesize enrollmentID=_enrollmentID;
@property(retain, nonatomic) NSString *personaID; // @synthesize personaID=_personaID;
@property(readonly, retain, nonatomic) NSString *enrollmentMode; // @synthesize enrollmentMode=_enrollmentMode;
@property(readonly, retain, nonatomic) NSString *assignedManagedAppleID; // @synthesize assignedManagedAppleID=_assignedManagedAppleID;
@property(readonly, retain, nonatomic) NSString *managedAppleID; // @synthesize managedAppleID=_managedAppleID;
@property(readonly, nonatomic) NSArray *serverCapabilities; // @synthesize serverCapabilities=_serverCapabilities;
@property(readonly, nonatomic) _Bool checkOutWhenRemoved; // @synthesize checkOutWhenRemoved=_checkOutWhenRemoved;
@property(readonly, nonatomic) _Bool signMessage; // @synthesize signMessage=_signMessage;
@property(readonly, nonatomic) int accessRights; // @synthesize accessRights=_accessRights;
@property(readonly, nonatomic) _Bool pinningRevocationCheckRequired; // @synthesize pinningRevocationCheckRequired=_pinningRevocationCheckRequired;
@property(retain, nonatomic) NSArray *checkInPinningPersistentRefs; // @synthesize checkInPinningPersistentRefs=_checkInPinningPersistentRefs;
@property(readonly, retain, nonatomic) NSArray *checkInPinningUUIDs; // @synthesize checkInPinningUUIDs=_checkInPinningUUIDs;
@property(readonly, retain, nonatomic) NSString *checkInURLString; // @synthesize checkInURLString=_checkInURLString;
@property(readonly, nonatomic) _Bool useDevelopmentAPNS; // @synthesize useDevelopmentAPNS=_useDevelopmentAPNS;
@property(retain, nonatomic) NSArray *serverPinningPersistentRefs; // @synthesize serverPinningPersistentRefs=_serverPinningPersistentRefs;
@property(readonly, retain, nonatomic) NSArray *serverPinningUUIDs; // @synthesize serverPinningUUIDs=_serverPinningUUIDs;
@property(readonly, retain, nonatomic) NSString *serverURLString; // @synthesize serverURLString=_serverURLString;
@property(readonly, retain, nonatomic) NSString *topic; // @synthesize topic=_topic;
@property(retain, nonatomic) NSData *identityPersistentID; // @synthesize identityPersistentID=_identityPersistentID;
@property(readonly, retain, nonatomic) NSString *identityUUID; // @synthesize identityUUID=_identityUUID;
@property(readonly, retain, nonatomic) NSString *managedAppleIDName;
@property(readonly, nonatomic) _Bool isUserEnrollment;
- (id)kvsForCertUUIDs:(id)arg1 persistentRefs:(id)arg2 labelKey:(id)arg3;	// IMP=0x0000000000042e7d
- (id)payloadDescriptionKeyValueSections;	// IMP=0x000000000004236e
- (id)subtitle1Description;	// IMP=0x0000000000042359
- (id)subtitle1Label;	// IMP=0x0000000000042321
- (id)installationWarnings;	// IMP=0x0000000000042005
@property(readonly, retain, nonatomic) NSArray *localizedAccessRightDescriptions;
- (id)verboseDescription;	// IMP=0x000000000004125e
- (id)stubDictionary;	// IMP=0x0000000000040edc
- (id)initWithDictionary:(id)arg1 profile:(id)arg2 outError:(id *)arg3;	// IMP=0x000000000003fe84
- (id)_nonHTTPSURLErrorForField:(id)arg1;	// IMP=0x000000000003fe00
- (id)_invalidTopicError;	// IMP=0x000000000003fd51
- (id)_invalidRightsError;	// IMP=0x000000000003fca2

@end

