//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class MPModelGenericObject;

@interface MPCSharedListeningQueueEventContentReplaced : NSObject
{
    MPModelGenericObject *_startItem;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x00000000000f7ed4
@property(readonly, copy, nonatomic) MPModelGenericObject *startItem; // @synthesize startItem=_startItem;
- (id)description;	// IMP=0x00000000000f7ea1
- (id)initWithStartItem:(id)arg1;	// IMP=0x00000000000f7e36

@end

