//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class MPModelGenericObject, NSArray;

@interface MPCSharedListeningQueueEventContentPlayedNow : NSObject
{
    MPModelGenericObject *_container;	// 8 = 0x8
    NSArray *_items;	// 16 = 0x10
    MPModelGenericObject *_startItem;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x00000000000f7ca7
@property(readonly, copy, nonatomic) MPModelGenericObject *startItem; // @synthesize startItem=_startItem;
@property(readonly, copy, nonatomic) NSArray *items; // @synthesize items=_items;
@property(readonly, nonatomic) MPModelGenericObject *container; // @synthesize container=_container;
- (id)description;	// IMP=0x00000000000f7c28
- (id)initWithItems:(id)arg1 container:(id)arg2 startItem:(id)arg3;	// IMP=0x00000000000f7b61

@end

