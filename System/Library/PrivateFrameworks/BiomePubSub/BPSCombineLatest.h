//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@protocol BPSPublisher;

@interface BPSCombineLatest
{
    id <BPSPublisher> _a;	// 8 = 0x8
    id <BPSPublisher> _b;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x000000000001e0c3
@property(readonly, nonatomic) id <BPSPublisher> b; // @synthesize b=_b;
@property(readonly, nonatomic) id <BPSPublisher> a; // @synthesize a=_a;
- (void)subscribe:(id)arg1;	// IMP=0x000000000001df5f
- (id)initWithA:(id)arg1 b:(id)arg2;	// IMP=0x000000000001debd
- (id)init;	// IMP=0x000000000001deb7

@end

