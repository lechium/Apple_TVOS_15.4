//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSArray, NSDictionary, NSString;

@interface PGUpNextMemoriesGenerationResult : NSObject
{
    NSArray *_memoryLocalIdentifiers;	// 8 = 0x8
    NSString *_debugInfo;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x000000000022b7ab
@property(readonly, nonatomic) NSString *debugInfo; // @synthesize debugInfo=_debugInfo;
@property(readonly, nonatomic) NSArray *memoryLocalIdentifiers; // @synthesize memoryLocalIdentifiers=_memoryLocalIdentifiers;
@property(readonly, nonatomic) NSDictionary *dictionaryRepresentation;
- (id)initWithMemoryLocalIdentifiers:(id)arg1 debugInfo:(id)arg2;	// IMP=0x000000000022b672

@end

