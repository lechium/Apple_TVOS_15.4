//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class PGGraphPersonNodeCollection;

@interface PGExcitingMomentsMemoryTitleGenerator
{
    PGGraphPersonNodeCollection *_personNodeAsCollection;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x000000000047d299
@property(retain, nonatomic) PGGraphPersonNodeCollection *personNodeAsCollection; // @synthesize personNodeAsCollection=_personNodeAsCollection;
- (void)_generateTitleAndSubtitleWithResult:(CDUnknownBlockType)arg1;	// IMP=0x000000000047cfa3

@end

