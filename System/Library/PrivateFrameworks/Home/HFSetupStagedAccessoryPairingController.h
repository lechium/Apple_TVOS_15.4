//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <Home/HFSetupPairingController-Protocol.h>

@class HFDiscoveredAccessory, HFSetupAccessoryResult, HFSetupPairingContext, HMAccessorySetupCompletedInfo, HMHome, NSHashTable, NSSet, NSString;

@interface HFSetupStagedAccessoryPairingController : NSObject <HFSetupPairingController>
{
    HFSetupPairingContext *_context;	// 8 = 0x8
    HFSetupAccessoryResult *_setupResult;	// 16 = 0x10
    unsigned long long _phase;	// 24 = 0x18
    HMHome *_home;	// 32 = 0x20
    NSHashTable *_pairingObservers;	// 40 = 0x28
    NSSet *_pairedAccessories;	// 48 = 0x30
    HMAccessorySetupCompletedInfo *_completedInfo;	// 56 = 0x38
    HFDiscoveredAccessory *_discoveredAccessoryToPair;	// 64 = 0x40
    NSString *_statusTitle;	// 72 = 0x48
    NSString *_statusDescription;	// 80 = 0x50
}

+ (_Bool)supportsSetupPayloadRetry;	// IMP=0x00000000002801ab
- (void).cxx_destruct;	// IMP=0x00000000002812f1
@property(retain, nonatomic) NSString *statusDescription; // @synthesize statusDescription=_statusDescription;
@property(retain, nonatomic) NSString *statusTitle; // @synthesize statusTitle=_statusTitle;
@property(retain, nonatomic) HFDiscoveredAccessory *discoveredAccessoryToPair; // @synthesize discoveredAccessoryToPair=_discoveredAccessoryToPair;
@property(retain, nonatomic) HMAccessorySetupCompletedInfo *completedInfo; // @synthesize completedInfo=_completedInfo;
@property(retain, nonatomic) NSSet *pairedAccessories; // @synthesize pairedAccessories=_pairedAccessories;
@property(retain, nonatomic) NSHashTable *pairingObservers; // @synthesize pairingObservers=_pairingObservers;
@property(retain, nonatomic) HMHome *home; // @synthesize home=_home;
@property(nonatomic) unsigned long long phase; // @synthesize phase=_phase;
@property(retain, nonatomic) HFSetupAccessoryResult *setupResult; // @synthesize setupResult=_setupResult;
@property(readonly, nonatomic) HFSetupPairingContext *context; // @synthesize context=_context;
- (id)cancel;	// IMP=0x0000000000280f2b
- (void)startWithHome:(id)arg1;	// IMP=0x0000000000280660
- (void)removePairingObserver:(id)arg1;	// IMP=0x00000000002805ee
- (void)addPairingObserver:(id)arg1;	// IMP=0x000000000028051b
- (id)initWithContext:(id)arg1;	// IMP=0x00000000002801b3

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

