//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class HMAccessory, HMSymptom, HMSymptomFixSession;

@interface HFSymptomFixManagerItem : NSObject
{
    HMSymptom *_symptom;	// 8 = 0x8
    HMSymptomFixSession *_fixSession;	// 16 = 0x10
    HMAccessory *_accessory;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x00000000001005d0
@property(readonly, nonatomic) HMAccessory *accessory; // @synthesize accessory=_accessory;
@property(readonly, nonatomic) HMSymptomFixSession *fixSession; // @synthesize fixSession=_fixSession;
@property(readonly, nonatomic) HMSymptom *symptom; // @synthesize symptom=_symptom;
- (id)description;	// IMP=0x0000000000100489
- (id)initWithSymptom:(id)arg1 fixSession:(id)arg2 accessory:(id)arg3;	// IMP=0x00000000001003d2

@end

