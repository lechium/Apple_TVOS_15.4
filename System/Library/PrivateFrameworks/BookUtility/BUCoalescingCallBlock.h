//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSMutableArray, NSString;
@protocol OS_dispatch_group, OS_dispatch_queue;

@interface BUCoalescingCallBlock : NSObject
{
    _Bool _pending;	// 8 = 0x8
    _Bool _invoking;	// 9 = 0x9
    unsigned short _notifyTimeoutDurationSeconds;	// 10 = 0xa
    double _coalescingDelay;	// 16 = 0x10
    double _maximumDelay;	// 24 = 0x18
    CDUnknownBlockType _notifyBlock;	// 32 = 0x20
    CDUnknownBlockType _notifyTimeoutBlock;	// 40 = 0x28
    unsigned long long _nextCoalescingTime;	// 48 = 0x30
    unsigned long long _nextMaxTime;	// 56 = 0x38
    NSString *_notifyBlockDescription;	// 64 = 0x40
    NSMutableArray *_callbacks;	// 72 = 0x48
    NSObject<OS_dispatch_queue> *_sync;	// 80 = 0x50
    NSObject<OS_dispatch_group> *_group;	// 88 = 0x58
}

- (void).cxx_destruct;	// IMP=0x0000000000010b17
@property(retain, nonatomic) NSObject<OS_dispatch_group> *group; // @synthesize group=_group;
@property(retain, nonatomic) NSObject<OS_dispatch_queue> *sync; // @synthesize sync=_sync;
@property(retain, nonatomic) NSMutableArray *callbacks; // @synthesize callbacks=_callbacks;
@property(copy, nonatomic) NSString *notifyBlockDescription; // @synthesize notifyBlockDescription=_notifyBlockDescription;
@property(nonatomic, getter=isInvoking) _Bool invoking; // @synthesize invoking=_invoking;
@property(nonatomic) _Bool pending; // @synthesize pending=_pending;
@property(nonatomic) unsigned long long nextMaxTime; // @synthesize nextMaxTime=_nextMaxTime;
@property(nonatomic) unsigned long long nextCoalescingTime; // @synthesize nextCoalescingTime=_nextCoalescingTime;
@property(nonatomic) unsigned short notifyTimeoutDurationSeconds; // @synthesize notifyTimeoutDurationSeconds=_notifyTimeoutDurationSeconds;
@property(copy, nonatomic) CDUnknownBlockType notifyTimeoutBlock; // @synthesize notifyTimeoutBlock=_notifyTimeoutBlock;
@property(copy, nonatomic) CDUnknownBlockType notifyBlock; // @synthesize notifyBlock=_notifyBlock;
@property(nonatomic) double maximumDelay; // @synthesize maximumDelay=_maximumDelay;
@property(nonatomic) double coalescingDelay; // @synthesize coalescingDelay=_coalescingDelay;
- (void)signalWithCompletion:(CDUnknownBlockType)arg1;	// IMP=0x000000000001051b
- (void)_coalescingTimeExpired:(unsigned long long)arg1;	// IMP=0x000000000001041c
- (void)_maxTimeExpired:(unsigned long long)arg1;	// IMP=0x00000000000103e4
- (void)_invoke;	// IMP=0x000000000000fb63
- (void)_invokeIfNotInProgress;	// IMP=0x000000000000faff
- (id)description;	// IMP=0x000000000000f9e7
- (id)initWithNotifyBlock:(CDUnknownBlockType)arg1 notifyTimeout:(unsigned short)arg2 blockDescription:(id)arg3 notifyTimeoutBlock:(CDUnknownBlockType)arg4;	// IMP=0x000000000000f8c2
- (id)initWithNotifyBlock:(CDUnknownBlockType)arg1 blockDescription:(id)arg2;	// IMP=0x000000000000f83f

@end

