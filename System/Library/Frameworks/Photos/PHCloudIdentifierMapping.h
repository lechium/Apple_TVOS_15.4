//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSError, PHCloudIdentifier;

@interface PHCloudIdentifierMapping : NSObject
{
    PHCloudIdentifier *_cloudIdentifier;	// 8 = 0x8
    NSError *_error;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x0000000000139a5a
@property(readonly, nonatomic) NSError *error; // @synthesize error=_error;
@property(readonly, nonatomic) PHCloudIdentifier *cloudIdentifier; // @synthesize cloudIdentifier=_cloudIdentifier;
- (id)initWithCloudIdentifier:(id)arg1 error:(id)arg2;	// IMP=0x00000000001399ad

@end

