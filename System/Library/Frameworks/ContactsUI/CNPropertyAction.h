//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <ContactsUI/CNContactActionProtocol-Protocol.h>

@class CNPropertyGroupItem, NSArray;
@protocol CNPropertyActionDelegate;

__attribute__((visibility("hidden")))
@interface CNPropertyAction <CNContactActionProtocol>
{
    NSArray *_propertyItems;	// 8 = 0x8
}

+ (void)performDefaultActionForItem:(id)arg1 sender:(id)arg2;	// IMP=0x0000000000078c21
- (void).cxx_destruct;	// IMP=0x0000000000079446
@property(copy, nonatomic) NSArray *propertyItems; // @synthesize propertyItems=_propertyItems;
- (void)presentDisambiguationAlertWithSender:(id)arg1;	// IMP=0x0000000000078f8f
- (void)performActionWithSender:(id)arg1;	// IMP=0x0000000000078ea2
- (void)performActionForItem:(id)arg1 sender:(id)arg2;	// IMP=0x0000000000078e3f
@property(readonly, nonatomic) _Bool canPerformAction;
@property(readonly, nonatomic) CNPropertyGroupItem *propertyItem;
- (id)initWithContact:(id)arg1 propertyItem:(id)arg2;	// IMP=0x0000000000078afd
- (id)initWithContact:(id)arg1 propertyItems:(id)arg2;	// IMP=0x0000000000078a79

// Remaining properties
@property(nonatomic) __weak id <CNPropertyActionDelegate> delegate; // @dynamic delegate;

@end

