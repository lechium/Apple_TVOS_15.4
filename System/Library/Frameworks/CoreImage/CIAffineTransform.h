//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <CoreImage/_CIFilterProperties-Protocol.h>

@class CIImage, NSValue;

__attribute__((visibility("hidden")))
@interface CIAffineTransform <_CIFilterProperties>
{
    CIImage *inputImage;	// 72 = 0x48
    NSValue *inputTransform;	// 80 = 0x50
}

+ (id)customAttributes;	// IMP=0x000000000000cc8a
@property(retain, nonatomic) NSValue *inputTransform; // @synthesize inputTransform;
@property(retain, nonatomic) CIImage *inputImage; // @synthesize inputImage;
- (id)_initFromProperties:(id)arg1;	// IMP=0x000000000000d1aa
- (id)_outputProperties;	// IMP=0x000000000000d07b
- (id)outputImage;	// IMP=0x000000000000ce27

@end

